...
Original file is located at
    https://colab.research.google.com/drive/17pmF-znL269teAFJB268ccDEApmmw325
"""
...
class TradeManager:
    ...
    def get_ticker_price(self, symbol):
        """Fetches live price safely."""
        symbol = symbol.replace("-", "/") # Format correction for CCXT
        if self.dry_run or self.exchange is None:
            # Fallback to Public API for price checking in Dry Run
            try:
                ex = ccxt.binance()
    ...
    def execute_order(self, symbol, side, amount_usd):
        ...
        if price <= 0:
            return "❌ Error: Could not verify market price."

        amount_coin = amount_usd / price

        # 1. Dry Run Logic
        if self.dry_run:
            log_entry = f"📝 [PAPER] {side.upper()} {amount_coin:.5f} {symbol} @ {format_currency(price)}"
        ...
        try:
            ...
            # Execute
            # Note: create_market_order automatically handles buy/sell string
            order = self.exchange.create_market_order(symbol, side, amount_coin)

            log_entry = f"🚀 [LIVE] {side.upper()} {symbol} | ID: {order['id']}"
            st.session_state['trade_log'].insert(0, f"{datetime.now().strftime('%H:%M:%S')} - {log_entry}")
